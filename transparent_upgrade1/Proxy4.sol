...
contract CounterV1{
    ...
    function getImplemenation() external pure returns(address) {
        return address(2);
    }
}
...
contract Proxy{
    //给对应的槽位指针赋值，防止操作地址冲突
    bytes32 private  constant IMPLEMENATION_SLOT = bytes32(uint(keccak256("eip1967.proxy.implementation"))-1);
    ...
    //目的是为了可以更换管理员地址，此处引入管理员合约的概念，
    //将这个地址设置成管理员合约，管理员来操作管理员合约
    //这样只有‘管理员合约’调用的时候走的是 proxy的getAdmin函数，其他用户
    //调用的都是实际的被代理的合约的getAdmin函数
    ...
    function getImplemenation() external ifAdmin returns(address) {
        return _getImplemenation();
    }
}
// 代理管理员的作用就是让所有访问代理合约的方法，访问的都是代理合约，
// 即使代理合约中有和proxy中相同的函数，也会绕开，而保证了代理管理员合约的地址为
// proxy的admin，可以 访问对应的管理函数
contract ProxyAdmin{
    ...
    function getProxyImplemenation(address proxy)external view returns(address){
        (bool ok,bytes memory res) = proxy.staticcall(
            abi.encodeCall(Proxy.getImplemenation,())
        );
        require(ok,"failed");
        return abi.decode(res, (address));
    }
    ...
    function upgrade(address payable proxy,address implemenation) 
    external onlyOwner{
        Proxy(proxy).upgradeTo(implemenation);
    }
